...
resource "aws_lb" "proj_lb" {
    ...
    subnet_mapping {
        subnet_id       = data.terraform_remote_state.rs_vpc.outputs.public_subnet_id[0]
    }
    subnet_mapping {
        subnet_id       = data.terraform_remote_state.rs_vpc.outputs.public_subnet_id[1]
    }    
    subnet_mapping {
        subnet_id       = data.terraform_remote_state.rs_vpc.outputs.public_subnet_id[2]
    }
}
...
resource "aws_autoscaling_group" "proj_asg" {
  ...
  max_size           = 6
  min_size           = 3
  vpc_zone_identifier       = [data.terraform_remote_state.rs_vpc.outputs.public_subnet_id[0], data.terraform_remote_state.rs_vpc.outputs.public_subnet_id[1], data.terraform_remote_state.rs_vpc.outputs.public_subnet_id[2]]
